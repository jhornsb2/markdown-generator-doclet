...
/**
 * Factory for creating {@link EnumData} instances from Java enum
 * {@link Element} values.
 */
@Value
public class EnumDataFactory {
	 ...
	 * is expected to be an enum element. The method first checks the cache for
	 * an existing instance and returns it if found. If not found, it creates a
	 * new instance, stores it in the cache, and then returns it.
	 *
	 * @param element The {@link Element} representing the enum for which to
	 *                create the {@link EnumData}. This element is expected to
	 *                be a {@link TypeElement}.
	 * @return A {@link EnumData} instance representing the enum.
	 */
	public EnumData create(@NonNull final Element element) {
		final String key = this.elementDataCache.keyFor(element);
		return (EnumData) this.elementDataCache.getOrCompute(key, () ->
			this.createUncached(element)
		);
	}

	/**
	 * Creates a {@link EnumData} instance for the given {@link Element}
	 ...
	 *
	 * @param element The {@link Element} representing the enum for which to
	 *                create the {@link EnumData}. This {@link Element} is
	 *                expected to be a {@link TypeElement}.
	 * @return A {@link EnumData} instance representing the enum.
	 */
	EnumData createUncached(@NonNull final Element element) {
	 ...
	 * enum does not implement any interfaces or if the interfaces cannot be
	 ...
	 *         superinterfaces implemented by the enum. If no superinterfaces
	 ...
	 * superclass. If the enum does not have a superclass or if the superclass

...
public class InterfaceElementProcessor implements IDocletElementProcessor {
	...
	private static final DocletLogger log = DocletLogger.forClass(
		InterfaceElementProcessor.class
	);
	...
	public String toMarkdownString() {
		log.debug(
			"Generating markdown for interface: {}",
			this.interfaceElement.getQualifiedName()
		);

		return InterfaceElementProcessor.TEMPLATE.replace(
			"${simpleName}",
			this.interfaceElement.getSimpleName().toString()
		)
			.replace("${kind}", this.interfaceElement.getKind().toString())
			.replace(
				"${packageName}",
				this.interfaceElement.getEnclosingElement().toString()
			)
			.replace(
				"${interfaces}",
				this.interfaceElement.getInterfaces().toString()
			)
			.replace(
				"${docComment}",
				this.docCommentUtil.getDocCommentTree(this.interfaceElement)

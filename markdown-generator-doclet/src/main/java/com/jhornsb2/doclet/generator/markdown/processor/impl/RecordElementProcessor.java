...
public class RecordElementProcessor implements IDocletElementProcessor {
	...
	public String getOutputFilepath() {
		...
		return (
			this.recordElement.getQualifiedName().toString().replace('.', '/') +
			"/index.md"
		);
	}
	...
	public String toMarkdownString() {
		log.debug(
			"Generating markdown for record: {}",
			this.recordElement.getQualifiedName()
		);

		return RecordElementProcessor.TEMPLATE.replace(
			"${simpleName}",
			this.recordElement.getSimpleName().toString()
		)
			.replace("${kind}", this.recordElement.getKind().toString())
			.replace(
				"${packageName}",
				this.recordElement.getEnclosingElement().toString()
			)
			.replace(
				"${superclassName}",
				this.recordElement.getSuperclass().toString()
			)
			.replace(
				"${interfaces}",
				this.recordElement.getInterfaces().toString()
			)
			.replace(
				"${docComment}",
				this.docCommentUtil.getDocCommentTree(this.recordElement)

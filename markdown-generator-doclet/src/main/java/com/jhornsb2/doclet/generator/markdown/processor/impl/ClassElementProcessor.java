...
public class ClassElementProcessor implements IDocletElementProcessor {
	...
	public String toMarkdownString() {
		log.debug(
			"Generating markdown for class: {}",
			this.classElement.getQualifiedName()
		);

		return ClassElementProcessor.TEMPLATE.replace(
			"${simpleName}",
			this.classElement.getSimpleName().toString()
		)
			.replace("${kind}", this.classElement.getKind().toString())
			.replace(
				"${packageName}",
				this.classElement.getEnclosingElement().toString()
			)
			.replace(
				"${superclassName}",
				this.classElement.getSuperclass().toString()
			)
			.replace(
				"${interfaces}",
				this.classElement.getInterfaces().toString()
			)
			.replace(
				"${docComment}",
	...
	public void processElement() throws IOException {
		...
		this.classElement.getEnclosedElements().forEach(element -> {
			...
			switch (element.getKind()) {
				...
				default:
					log.warn(
						"{}: Skipping processing for element: {}",
